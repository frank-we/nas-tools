...
class WordsHelper:
    ...
    def init_config(self):
        ...
        self.ignored_words_info = self.dbhelper.get_custom_words(gid=-1,
                                                                 enabled=1,
                                                                 wtype=1,
                                                                 regex=1)
        self.ignored_words_noregex_info = self.dbhelper.get_custom_words(
            gid=-1, enabled=1, wtype=1, regex=0)
        self.replaced_words_info = self.dbhelper.get_custom_words(gid=-1,
                                                                  enabled=1,
                                                                  wtype=2,
                                                                  regex=1)
        self.replaced_words_noregex_info = self.dbhelper.get_custom_words(
            gid=-1, enabled=1, wtype=2, regex=0)
        self.replaced_offset_words_info = self.dbhelper.get_custom_words(
            gid=-1, enabled=1, wtype=3, regex=1)
        self.offset_words_info = self.dbhelper.get_custom_words(gid=-1,
                                                                enabled=1,
                                                                wtype=4,
                                                                regex=1)
