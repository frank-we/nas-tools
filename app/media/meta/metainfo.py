...
def MetaInfo(title, subtitle=None, mtype=None, customWordGroupId=None):
    """
    ...
    if customWordGroupId:
        title, msg, used_info = WordsHelper().processByGid(
            title=title, gid=customWordGroupId)
        if subtitle:
